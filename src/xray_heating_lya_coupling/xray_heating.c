...
void xray_build_filter_functions(xray_grid_t *thisXray_grid, double *xray_filter_heating, double *xray_filter_ionization, xray_params_t *xray_params, int type)
{
    ...
    for(int i=0; i<nbins; i++)
    {
        ...
        xray_params->x = x;              //distance from source in comoving cm
...
//         double const vol_emission = CUB((thisXray_grid->box_size/h)/nbins);
        ...
        debug_printf(DEBUG_XRAY_FILTER, "+DEBUG+ (1.+z)^2/(4.*M_PI*distance(in cmoving cm)^2) = %e\n", SQR(1.+z)/(4.*M_PI*SQR(x + eps)));
        debug_printf(DEBUG_XRAY_FILTER, "+DEBUG+ xray_heating_HI_calc_integral(*xray_params, nu_HI, nu_HI*1.e4) = %e\n", xray_heating_HI_calc_integral(*xray_params, nu_HI, nu_HI*1.e4));
        ...
        debug_printf(DEBUG_XRAY_FILTER, "+DEBUG+ x = %e cm\t z_em = %e\t heating[%d] = %e\t ionization[%d] = %e\n", x, tmp-1., i, xray_filter_heating[i], i, xray_filter_ionization[i]);
    }
}
...
void xray_heating_and_ionization(xray_grid_t *thisXray_grid, cosmology_t *thisCosmology, double Xe, xray_spectrum_t *thisSpectrum, int myRank)
{
    ...
    if(myRank == 0) printf("   xray heating and ionization: initialisation done\n");
    ...
    {
        if(myRank == 0) printf("   building filter function for type %d\n", type);
        ...
        struct stat st = {0};
        if ((stat("output", &st) == -1) && myRank == 0) 
        {
            printf("+DEBUG+ creating directory\n");
            mkdir("output", 0755);
        }
#ifdef __MPI
            write_grid_to_file_float(xray_filter_heating, nbins, local_n0, thisXray_grid->local_0_start, "output/xray_heating_filter.dat");
            write_grid_to_file_float(thisXray_grid->xray_lum, nbins, local_n0, thisXray_grid->local_0_start, "output/xray_lum_HI.dat");
            write_grid_to_file_float(thisXray_grid->xray_heating_HI, nbins, local_n0, thisXray_grid->local_0_start, "output/xray_heating_HI.dat");
#else
            write_grid_to_file_float(xray_filter_heating, nbins, local_n0, "output/xray_heating_filter.dat");
            write_grid_to_file_float(thisXray_grid->xray_lum, nbins, local_n0, "output/xray_heating_lum.dat");
            write_grid_to_file_float(thisXray_grid->xray_heating_HI, nbins, local_n0, "output/xray_heating_HI.dat");
#endif
#endif
        }else if(type == 1){
...
void xray_heating_and_ionization_global(xray_grid_t *thisXray_grid, cosmology_t *thisCosmology, double Xe, xray_spectrum_t *thisSpectrum, int myRank)
{
    ...
    xray_heating_and_ionization(thisXray_grid, thisCosmology, Xe, thisSpectrum, myRank);
...
xray_grid_t *allocate_xray_grid(int nbins, float box_size)
{
    ...
    initialize_xray_grid(thisXray_grid->xray_heating_HI, nbins, local_n0, 0.);
    initialize_xray_grid(thisXray_grid->xray_heating_HeI, nbins, local_n0, 0.);
    initialize_xray_grid(thisXray_grid->xray_heating_HeII, nbins, local_n0, 0.);

    initialize_xray_grid(thisXray_grid->xray_ionization_HI, nbins, local_n0, 0.);
    initialize_xray_grid(thisXray_grid->xray_ionization_HeI, nbins, local_n0, 0.);
    initialize_xray_grid(thisXray_grid->xray_ionization_HeII, nbins, local_n0, 0.);

    initialize_xray_grid(thisXray_grid->xray_lum, nbins, local_n0, 0.);
...
void read_lum_xraygrid(xray_grid_t *thisXray_grid, char *filename, int double_precision)
{
    ...
    for(int i=0; i<local_n0; i++)
    {
        for(int j=0; j<nbins; j++)
        {
            for(int k=0; k<nbins; k++)
            {
                if(creal(thisXray_grid->xray_lum[i*nbins*nbins+j*nbins+k])>0.)
                {
                    printf("+SHOW+ %d %d %d:\t%e\n", i+local_n0,j,k,creal(thisXray_grid->xray_lum[i*nbins*nbins+j*nbins+k]));
                }
            }
        }
    }
