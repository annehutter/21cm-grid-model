...
grid_21cm_t *init21cmgrid()
{
	...
	this21cmGrid->z = 0.;
	this21cmGrid->z_old = 0.;
	
	this21cmGrid->temp_old = NULL;
	this21cmGrid->dens_old = NULL;
	this21cmGrid->Xe_old = NULL;
...
{
	if(this21cmGrid->temp_old != NULL) fftw_free(this21cmGrid->temp_old);
	if(this21cmGrid->dens_old != NULL) fftw_free(this21cmGrid->dens_old);
	if(this21cmGrid->Xe_old != NULL) fftw_free(this21cmGrid->Xe_old);
...
grid_21cm_t *allocate_21cmgrid(int nbins, float box_size)
{
	...
	this21cmGrid->temp_old = fftw_alloc_complex(alloc_local);
	this21cmGrid->dens_old = fftw_alloc_complex(alloc_local);
	this21cmGrid->Xe_old = fftw_alloc_complex(alloc_local);
...
#else
	this21cmGrid->temp_old = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*local_n0*nbins*nbins);
	this21cmGrid->dens_old = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*local_n0*nbins*nbins);
	this21cmGrid->Xe_old = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*local_n0*nbins*nbins);
	...
	initialize_21cmgrid(this21cmGrid->temp_old, nbins, local_n0, 0.);
	initialize_21cmgrid(this21cmGrid->dens_old, nbins, local_n0, 1.);
	initialize_21cmgrid(this21cmGrid->Xe_old, nbins, local_n0, 1.e-10);

	initialize_21cmgrid(this21cmGrid->temp, nbins, local_n0, 0.);
	initialize_21cmgrid(this21cmGrid->dens, nbins, local_n0, 1.);
	initialize_21cmgrid(this21cmGrid->Xe, nbins, local_n0, 1.e-10);
	initialize_21cmgrid(this21cmGrid->XHI, nbins, local_n0, 1.);
	initialize_21cmgrid(this21cmGrid->XHeI, nbins, local_n0, 1.);
	initialize_21cmgrid(this21cmGrid->XHeII, nbins, local_n0, 1.e-10);
...
void overwrite_old_21cmgrid(grid_21cm_t *this21cmGrid)
{
	...
	for(int i=0; i<local_n0; i++)
	{
		for(int j=0; i<nbins; j++)
		{
			for(int k=0; k<nbins; k++)
			{
				this21cmGrid->temp_old[i*nbins*nbins+j*nbins+k] = creal(this21cmGrid->temp[i*nbins*nbins+j*nbins+k]) + 0.*I;
				
				this21cmGrid->dens_old[i*nbins*nbins+j*nbins+k] = creal(this21cmGrid->dens[i*nbins*nbins+j*nbins+k]) + 0.*I;
				
				this21cmGrid->Xe_old[i*nbins*nbins+j*nbins+k] = creal(this21cmGrid->Xe[i*nbins*nbins+j*nbins+k]) + 0.*I;
			}
		}
	}
}
...
#ifdef __MPI
	int local_0_start = this21cmGrid->local_0_start;
	
	write_grid_to_file_float(this21cmGrid->Tb, nbins, local_n0, local_0_start, filename);
#else
	write_grid_to_file_float(this21cmGrid->Tb, nbins, local_n0, filename);
#endif
}
...
void write_Ts_field_file(grid_21cm_t *this21cmGrid, char *filename)
{
	...
	write_grid_to_file_float(this21cmGrid->Ts_inv, nbins, local_n0, local_0_start, filename);
#else
	write_grid_to_file_float(this21cmGrid->Ts_inv, nbins, local_n0, filename);
#endif
}
...
void write_Xe_field_file(grid_21cm_t *this21cmGrid, char *filename)
{
	...
	write_grid_to_file_float(this21cmGrid->Xe, nbins, local_n0, local_0_start, filename);
#else
	write_grid_to_file_float(this21cmGrid->Xe, nbins, local_n0, filename);
#endif
}
...
void write_temp_field_file(grid_21cm_t *this21cmGrid, char *filename)
{
	...
	write_grid_to_file_float(this21cmGrid->temp, nbins, local_n0, local_0_start, filename);
#else
	write_grid_to_file_float(this21cmGrid->temp, nbins, local_n0, filename);
#endif
}