    ...
    for(int i=0; i<local_n0; i++)
    {
        ...
        for(int j=0; j<nbins; j++)
        {
            ...
            for(int k=0; k<nbins; k++)
            {
                ...
                debug_printf(DEBUG_LYA_FILTER, "+DEBUG+ nu = %e\t distance = %e cMpc\t 1 - [(1+z_emission)/(1+z)]^2 = %e\n", nu, sqrt(expr), h*100.*km_cm/Mpc_cm*sqrt(omega_m*(1.+z))/(2.*clight_cm)*sqrt(expr));
                debug_printf(DEBUG_LYA_FILTER, "+DEBUG+ %d %d %d: lya_filter = %e Hz^-1 s^-1 cm^-3 cm^-2\n", i, j, k, creal(filter[i*nbins*nbins+j*nbins+k]));
            }
        }
    }
}
...
double get_mean_lya_lyagrid(lya_grid_t *thisLya_grid)
{
    ...
    for(int i=0; i<local_n0; i++)
    {
        for(int j=0; j<nbins; j++)
        {
            for(int k=0; k<nbins; k++)
            {
                sum += creal(thisLya_grid->lya[i*nbins*nbins+j*nbins+k]);
            }
        }
    }
#ifdef __MPI
