...
void convolve_fft(xray_grid_t *thisXray_grid, fftw_complex *filter, fftw_complex *output, fftw_complex *input)
{
	int nbins;
	double factor;
#ifdef __MPI
	...
	assert(local_0_start == thisXray_grid->local_0_start);
	assert(local_n0 == thisXray_grid->local_n0);
